...
TArray<FStageNpc> UGameFlowSubsystem::GetResidentNpcs(FName LevelPath)
{
    ...
    if (Lv.Contains(TEXT("L01_Jangteo")))
        return {
            // 2026-07-25 좌표 재배치 — 옛 값(X 800~1750 · Y ±320)은 소형 거리 시절 것이라
            // 새 대로(X ±14000 · 차도 |Y|<900 · 인도 900~1250)에선 동쪽 끝에 몰리고 둘은 차도 한복판이었다.
            // 인도(|Y| 1020)에 서서 도로를 본다. 빌더가 놓는 NPC(X -9500·-4000·3000·10500)와 겹치지 않게 벌린다.
            N(TEXT("좌판 아낙"), -11000.f, 1020.f, 270.f, {
                TEXT("오늘 물건은 아침에 다 나갔어. 소금이 특히 빨리 빠지더라고."),
                TEXT("요샌 다들 뭐라도 쟁여 두려고 해. 불안하면 손이 그렇게 움직이지.") }),
            N(TEXT("짐꾼 노인"), -500.f, -1020.f, 90.f, {
                TEXT("신항 쪽 일이 늘었어. 파낸 걸 나르는 손이 모자란대."),
                TEXT("나이 든 나까지 부르는 걸 보면, 급하긴 급한 모양이지.") }),
            N(TEXT("떡집 딸"), 7500.f, 1020.f, 270.f, {
