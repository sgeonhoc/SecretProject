...
// html 은 웹/ 에, md 는 기획/ 에 있다 (2026-07-25 루트 정리)
function read(name){
  const p = name.endsWith(".html") ? path.join(DIR, "웹", name) : path.join(DIR, name);
...
fs.writeFileSync(path.join(DIR, "웹", "세계관_지도_문서_데이터.js"), out, "utf8");
